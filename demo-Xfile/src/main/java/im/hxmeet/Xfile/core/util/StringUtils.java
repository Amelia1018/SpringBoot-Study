...
public class StringUtils {
     ...
     *
     */
    private static String trimStartSlashes(String path) {
        if(StrUtil.isEmpty(path)){
            return path;
        }
        //检查 path 是否以 DELIMITER_STR 开头
        while(path.startsWith(DELIMITER_STR)){
            //将 path 更新为去掉第一个字符的子字符串（通过 path.substring(1)）。
            //这将不断循环，直到 path 不再以 DELIMITER_STR 开头。
    ...
    public static String removeDuplicateSlashes(String path) {
        ...
        for (int i = sb.length(); i < path.length() - 1; i++) {
            ...
            //current 和 next 只有一个是'/'，才能加到path中
            if (!(current == DELIMITER && next == DELIMITER)) {
                sb.append(current);
            }
        }
        //把最后一个'/'加到中间去重的path中，这才完整
    ...
    public static String concat(String... strs) {
        ...
        //调用 removeDuplicateSlashes调用去除路径中的所有重复斜杠。
