package im.hxmeet.Xfile.module.config.service;
...
import im.hxmeet.Xfile.core.config.ZFileProperties;
import im.hxmeet.Xfile.core.util.EnumConvertUtils;
import im.hxmeet.Xfile.module.config.event.ISystemConfigModifyHandler;
import im.hxmeet.Xfile.module.config.mapper.SystemConfigMapper;
...
public class SystemConfigService {
     ...
     *          系统设置 dto
     */
    @Transactional(rollbackFor = Exception.class)// 注解表明该方法是一个事务性操作，如果出现异常则会回滚事务，确保数据一致性。
    @CacheEvict(allEntries = true)//注解表示在方法执行后，清空所有缓存数据，以确保客户端获取到最新的数据。
    ...
    public void resetAdminLoginInfo() {
        ...
        systemConfigMapper.updateById(loginVerifyModeConfig);//通过 updateById 方法更新数据库。
